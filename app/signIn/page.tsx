...
export default function SignIn() {
  ...
  const { signIn,user,SigninWithPopup } = useContext(AuthContext);
  ...
  const handlepopup = async () => {
    try {
      ...
      console.error('Sign-in error caught in handlepopup:', error);
    }
  };
  return (
    <div>
          <>
     <div className="flex-1 xl:pt-[5.5rem]" style={{ textDecoration: "initial" }}>
  <div
    className="flex max-w-lg mx-auto my-16 overflow-hidden bg-white rounded-lg lg:space-x-8 dark:bg-[#2A1A17] lg:max-w-5xl"
  ...
  >
    ...
    >
      <h2
        className="text-2xl font-semibold text-center text-gray-700 dark:text-white"
        style={{ textDecoration: "initial" }}
      >
        Welcome Back
      </h2>{" "}
      ...
      <div className="mt-4 space-y-3 sm:flex sm:items-center sm:space-x-4 sm:space-y-0">
        ...
        >
          ...
          <button onClick={handlepopup} className="text-sm text-gray-800 dark:text-gray-200">
            Login with Github
          </button>
        </div>
        <a
      ...
      <form onSubmit={handleSignIn}> 
        <input
          type="hidden"
          name="_token"
          defaultValue="XR4P98WeaYMLZlgZiddJScbdosHkKXY3cGbuLDqI"
          autoComplete="off"
        />{" "}
        <div className="mt-4" style={{ textDecoration: "initial" }}>
